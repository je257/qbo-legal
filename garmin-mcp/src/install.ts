...
export function runInstall(): void {
  ...
  servers.garmin = { command: nodePath, args: [serverPath] };
  config.mcpServers = servers;
  writeFileSync(configPath, JSON.stringify(config, null, 2) + "\n");

  console.log(`\nAdded the "garmin" Garmin Connect connector to Claude Desktop:`);
  console.log(`  ${configPath}`);
  console.log(`\nNow fully quit Claude Desktop and open it again.`);
  console.log(`Then try asking Claude: "Use garmin_daily_summary to show my stats for today."`);
  console.log(`\nUsing Claude Code instead? Copy and run this one line:`);
  console.log(`  claude mcp add garmin -- "${nodePath}" "${serverPath}"`);
}
